...
const JWT_SECRET = process.env.JWT_SECRET || 'revenge_jwt_secret_v2';
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // Allow the production domain if set
    const allowed = process.env.CLIENT_URL
    if (allowed && origin === allowed) return callback(null, true)
    callback(null, false)
  },
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization', 'x-admin-key', 'x-admin-pseudo'],
  credentials: true
}));
...
const WEAPON_VOTE_TIMEOUT = 15000; // 15s pour voter
const CAPTAIN_DISCUSS_TIMEOUT = 10000; // 10s de discussion avant le vote d'arme
const BAN_TIMEOUT = 15000;
const START_COUNTDOWN = 5000;
const ADMIN_PSEUDOS = ['Karim34', 'Biscuit', 'Serokku']; // ← ajoute ton pseudo ici
const CONTENT_PSEUDOS = []; // ← pseudos avec le rôle Content
// Rebuild from DB on boot so role survives restart
function rebuildContentPseudos() {
  try {
    const dbData = db.loadDB();
    (dbData.users || []).filter(u => u.isContent).forEach(u => {
      if (!CONTENT_PSEUDOS.includes(u.pseudo)) CONTENT_PSEUDOS.push(u.pseudo);
    });
    if (CONTENT_PSEUDOS.length) console.log('[Content] Loaded from DB:', CONTENT_PSEUDOS);
  } catch(e) { console.error('[Content] rebuild error:', e.message); }
}
rebuildContentPseudos(); // rebuilds from DB on boot
...
app.get('/api/profile/:pseudo', (req, res) => {
  ...
  res.json({ id: user.id, pseudo: user.pseudo, stats: user.stats, avatar: user.avatar || null, banned: !!user.banned, muted: !!user.muted, createdAt: user.createdAt, matchHistory: (user.matchHistory || []).slice(0, 20) });
});

// Admin actions
const ADMIN_KEY = process.env.ADMIN_KEY || 'revenge_admin_secret';

// Normalize IP: take first part of x-forwarded-for, trim whitespace, lowercase
function normalizeIp(raw) {
  if (!raw) return null;
  return raw.toString().split(',')[0].trim().toLowerCase() || null;
}
...
app.post('/api/admin/elo', express.json(), (req, res) => {
  ...
  // Sync stats to PG immediately
  if (process.env.DATABASE_URL) {
    db.syncStatsToPg(user.id, user.stats).catch(e => console.error('[PG] ELO sync:', e.message));
  }
...
app.get('/api/admin/smurfs', (req, res) => {
  ...
  try {
    ...
    (data.users || []).forEach(u => {
      // Normalize IP consistently with login/register
      const rawIp = normalizeIp(u.ip);
      const ip = rawIp || '__no_ip__';
      if (!byIp[ip]) byIp[ip] = [];
      const maxElo = Math.max(...['1v1','2v2','3v3','5v5'].map(m => u.stats?.[m]?.elo || 500));
      byIp[ip].push({ id: u.id, pseudo: u.pseudo, maxElo, banned: !!u.banned, createdAt: u.createdAt, rawIp: u.ip || null });
    });
    const suspects = [];
    Object.entries(byIp).forEach(([ip, accounts]) => {
      if (accounts.length < 2) return;
      suspects.push({ ip, accounts, count: accounts.length, noIp: ip === '__no_ip__' });
    });
...
app.get('/api/admin/find-user', (req, res) => {
  ...
  const sameIpAccounts = (!isTargetAdmin && user.ip)
    ? data.users.filter(u => u.id !== user.id && normalizeIp(u.ip) === normalizeIp(user.ip)).map(u => ({ id: u.id, pseudo: u.pseudo, createdAt: u.createdAt, banned: !!u.banned, stats: u.stats }))
    : [];
...
app.post('/api/admin/premium', (req, res) => {
  ...
  if (pu) {
    ...
    if (u) {
      if (pricePaid) u.premiumPaidAmount = (u.premiumPaidAmount || 0) + pricePaid;
      // Rang Fondateur : attribué au premier achat premium, permanent
      if (!u.isFondateur) {
        u.isFondateur = true;
        u.fondateurDate = new Date().toISOString();
      }
      db.saveDB(data);
      // Sync isFondateur + fondateurDate to PostgreSQL immediately
      if (process.env.DATABASE_URL) {
        const { Pool } = require('pg');
        const pgPool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
        pgPool.query(
          'UPDATE users SET is_fondateur = $1, fondateur_date = $2, premium_paid_amount = $3 WHERE id = $4',
          [!!u.isFondateur, u.fondateurDate || null, u.premiumPaidAmount || 0, userId]
        ).catch(e => console.error('[PG] fondateur sync:', e.message))
         .finally(() => pgPool.end());
      }
    }
  }
...
app.post('/api/admin/set-content', (req, res) => {
  ...
  if (!CONTENT_PSEUDOS.includes(pseudo)) CONTENT_PSEUDOS.push(pseudo);
  // Save to DB JSON
  const dbData = db.loadDB();
  const dbUser = dbData.users.find(u => u.pseudo === pseudo);
  if (dbUser) {
    dbUser.isContent = true;
    db.saveDB(dbData);
  }
  // Save to PostgreSQL
  if (process.env.DATABASE_URL) {
    db.sql('UPDATE users SET is_content = true WHERE pseudo = $1', [pseudo])
      .catch(e => console.error('[PG] set-content:', e.message));
  }
  // Update active socket
  io.sockets.sockets.forEach(s => {
    if (s.pseudo === pseudo) { s.isContent = true; s.emit('content_granted'); }
  });
  console.log('[Content] Granted to:', pseudo);
...
app.post('/api/admin/revoke-content', (req, res) => {
  ...
  if (idx !== -1) CONTENT_PSEUDOS.splice(idx, 1);
  // Remove from DB JSON
  const dbData = db.loadDB();
  const dbUser = dbData.users.find(u => u.pseudo === pseudo);
  if (dbUser) { dbUser.isContent = false; db.saveDB(dbData); }
  // Remove from PG
  if (process.env.DATABASE_URL) {
    db.sql('UPDATE users SET is_content = false WHERE pseudo = $1', [pseudo])
      .catch(e => console.error('[PG] revoke-content:', e.message));
  }
  io.sockets.sockets.forEach(s => {
...
app.get('/api/admin/content-list', (req, res) => {
  if (!isAdminReq(req)) return res.status(403).json({ error: 'Interdit' });
  // Read from DB to stay accurate after redeployment
  try {
    const dbData = db.loadDB();
    const fromDb = (dbData.users || []).filter(u => u.isContent).map(u => u.pseudo);
    // Merge with in-memory list
    const merged = [...new Set([...CONTENT_PSEUDOS, ...fromDb])];
    res.json({ contentPseudos: merged });
  } catch(e) {
...
app.post('/api/admin/reset-all', async (req, res) => {
  if (!isAdminReq(req)) return res.status(403).json({ error: 'Interdit' });
  const { type } = req.body;
  if (!['elo','stats','history','all'].includes(type)) return res.status(400).json({ error: 'Type invalide' });
  ...
  try {
    ...
    for (const user of dbData.users) {
      ...
      const modes = ['1v1','2v2','3v3','5v5'];
      if (!user.stats) user.stats = db.defaultStats();
      if (type === 'elo' || type === 'all') {
        modes.forEach(m => { if (user.stats[m]) user.stats[m].elo = 500; else user.stats[m] = { elo:500, wins:0, losses:0 }; });
      }
      if (type === 'stats' || type === 'all') {
        modes.forEach(m => { user.stats[m] = { elo: type==='all'?500:(user.stats[m]?.elo||500), wins:0, losses:0, currentStreak:0, bestStreak:0 }; });
      }
    ...
    if (process.env.DATABASE_URL) {
      for (const user of dbData.users) {
        if (user.isBot || !user.id) continue;
        await db.sql('UPDATE users SET stats = $1, match_history = $2 WHERE id = $3',
          [JSON.stringify(user.stats), JSON.stringify(user.matchHistory || []), user.id]
        ).catch(() => {});
      }
    }
    ...
    io.sockets.sockets.forEach(s => {
      if (s.userId) {
        const u = dbData.users.find(u => u.id === s.userId);
        if (u) s.emit('season_reset', { type, stats: u.stats });
      }
    });

    // Reset season countdown (keep same season number)
    if (type === 'all' || type === 'elo') {
      const dbData2 = db.loadDB();
      dbData2.season = {
        number: dbData2.season?.number || 1,
        startedAt: new Date().toISOString(),
        endsAt: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString()
      };
      db.saveDB(dbData2);
      io.sockets.sockets.forEach(s => s.emit('season_new', { number: dbData2.season.number }));
    }
    db.addAdminLog(req.headers['x-admin-pseudo']||'Admin', 'RESET', 'ALL', type + ' reset on ' + count + ' players');
    console.log('[RESET]', type, 'on', count, 'players');
...
app.post('/api/admin/delete-user', (req, res) => {
  ...
  try {
    ...
    if (ADMIN_PSEUDOS.includes(user.pseudo)) return res.status(403).json({ error: 'Impossible de supprimer un compte admin' });
    // Disconnect user if online
    io.sockets.sockets.forEach(s => {
      if (s.userId === userId) {
        s.emit('account_deleted', { message: 'Ton compte a été supprimé par un administrateur.' });
        s.disconnect(true);
      }
    });
    db.addAdminLog(req.headers['x-admin-pseudo'] || 'Admin', 'DELETE', user.pseudo, 'Compte supprimé');
    data.users.splice(idx, 1);
    db.saveDB(data);
    // Also delete from PostgreSQL so it doesn't come back on redeploy
    if (process.env.DATABASE_URL) {
      const { Pool } = require('pg');
      const pgPool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
      pgPool.query('DELETE FROM users WHERE id = $1', [userId])
        .catch(e => console.error('[PG] delete user error:', e.message))
        .finally(() => pgPool.end());
    }
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
...
app.post('/api/admin/add-coins', (req, res) => {
  ...
  if (!adminKey || adminKey !== ADMIN_KEY) return res.status(403).json({ error: 'Clé admin invalide' });
  const { userId, amount } = req.body;
  if (!userId || amount === undefined) return res.status(400).json({ error: 'userId et amount requis' });
  const parsedAmount = parseInt(amount);
  if (isNaN(parsedAmount)) return res.status(400).json({ error: 'Montant invalide' });
  const data = db.loadDB();
  const user = data.users.find(u => u.id === userId);
  if (!user) return res.status(404).json({ error: 'Joueur introuvable' });
  user.coins = Math.max(0, (user.coins || 0) + parsedAmount);
  db.saveDB(data);
  // Notify player in real time if online
  const ps = [...io.sockets.sockets.values()].find(s => s.userId === userId);
  if (ps) ps.emit('coins_updated', { coins: user.coins, change: parsedAmount, source: 'admin' });
...
app.get('/api/tickets/user/:userId', (req, res) => {
  const tickets = db.getTickets().filter(t => t.userId === req.params.userId);
  res.json(tickets);
});
...
app.post('/api/avatar', express.json({ limit: '5mb' }), (req, res) => {
...
app.post('/api/banner', express.json({ limit: '5mb' }), (req, res) => {
...
function resolveWeaponVote(roomId) {
  ...
  // Check if voters are premium — check room.teams AND live socket + DB as fallback
  const voter1Id = room.weaponVoters?.team1;
  const voter2Id = room.weaponVoters?.team2;
  const voter1 = voter1Id ? [...(room.teams[0]||[])].find(p => p.id === voter1Id) : null;
  const voter2 = voter2Id ? [...(room.teams[1]||[])].find(p => p.id === voter2Id) : null;
  // Fallback: check live socket isPremium or DB
  const getIsPremium = (voterId, voter) => {
    if (voter?.isPremium) return true;
    const s = [...io.sockets.sockets.values()].find(s => s.userId === voterId);
    if (s?.isPremium) return true;
    const u = db.getUserById(voterId);
    return !!u?.isPremium;
  };
  const p1Premium = voter1Id ? getIsPremium(voter1Id, voter1) : false;
  const p2Premium = voter2Id ? getIsPremium(voter2Id, voter2) : false;
  let chosen, msg;
  if (v1 && v2 && v1 === v2) {
    // Both agree → weapon chosen
    chosen = v1;
    msg = `✅ Accord trouvé ! Arme jouée : **${chosen}**`;
  } else if (p1Premium && !p2Premium && v1) {
    // Only team1 is premium → their weapon wins
    chosen = v1;
    msg = `👑 Arme choisie par un joueur Premium : **${chosen}**`;
  } else if (p2Premium && !p1Premium && v2) {
    // Only team2 is premium → their weapon wins
    chosen = v2;
    msg = `👑 Arme choisie par un joueur Premium : **${chosen}**`;
  } else {
    // Both premium, both non-premium, or no vote → random
    chosen = v1 || v2 ? WEAPONS[Math.floor(Math.random() * WEAPONS.length)] : WEAPONS[Math.floor(Math.random() * WEAPONS.length)];
    msg = (p1Premium && p2Premium && v1 && v2)
      ? `🎲 Deux joueurs Premium en désaccord — arme choisie aléatoirement : **${chosen}**`
...
const QUEUE_COOLDOWN_MS = 30 * 1000; // 30 secondes de cooldown après une partie

function emitGameResult(roomId, payload) {
  console.log('[GAME_RESULT] roomId:', roomId, '| winner:', payload.winner, '| winTeam:', payload.winTeam);
  const room = rooms[roomId];
  // ── Coin wager resolution ──
  if (room && room.wager > 0 && payload.winner > 0 && !payload.draw) {
    const winTeamArr = payload.winner === 1 ? (room.teams[0] || []) : (room.teams[1] || []);
    const loseTeamArr = payload.winner === 1 ? (room.teams[1] || []) : (room.teams[0] || []);
    const wagerData = db.loadDB();
    loseTeamArr.filter(p => !p.isBot && p.id).forEach(p => {
      const u = wagerData.users.find(x => x.id === p.id);
      if (u) u.coins = Math.max(0, (u.coins || 0) - room.wager);
    });
    winTeamArr.filter(p => !p.isBot && p.id).forEach(p => {
      const u = wagerData.users.find(x => x.id === p.id);
      if (u) u.coins = (u.coins || 0) + room.wager;
    });
    db.saveDB(wagerData);
    payload.wager = room.wager;
  }
  // Broadcast to socket room
  io.to('room_' + roomId).emit('game_result', payload);
  // Backup : envoyer directement à chaque joueur via socketId ET via userId
  if (room) {
    const allPlayers = [...(room.teams[0] || []), ...(room.teams[1] || [])].filter(p => !p.isBot);
    // Set cooldown on all player sockets + notify
    allPlayers.filter(p => p.id).forEach(p => {
      io.sockets.sockets.forEach(s => {
        if (s.userId === p.id) {
          s.emit('game_result', payload);
          // Set cooldown
          s.lastMatchEndedAt = Date.now();
          s.emit('queue_cooldown', { endsAt: Date.now() + QUEUE_COOLDOWN_MS });
        }
      });
    });
    // Par socketId stocké dans room
...
function startBanTimer(roomId) {
  ...
  room.banTimer = setTimeout(() => {
    ...
    emitToRoom(roomId, 'map_banned', { team: teamNum, map: autoMap, remainingMaps: newRemaining, auto: true });
    emitToRoom(roomId, 'chat_msg', { author: 'Système', team: 'system', text: `⏱️ Équipe ${teamNum} n'a pas banni — ${autoMap} auto-banni !` });
    ...
    if (newRemaining.length === 1) {
      ...
      emitToRoom(roomId, 'map_chosen', { map: chosen });
      emitToRoom(roomId, 'chat_msg', { author: 'Système', team: 'system', text: `🗺️ Map jouée : ${chosen} — GO !` });
      // ── Room inactivity timeout (90min) ──
      scheduleRoomTimeout(roomId);
    } else {
      const nextTeam = room.banTurn % 2 === 0 ? 1 : 2;
      emitToRoom(roomId, 'ban_phase', { turn: room.banTurn, team: nextTeam, mapsLeft: newRemaining.length });
      startBanTimer(roomId);
    }
  }, BAN_TIMEOUT);

  emitToRoom(roomId, 'ban_timer_start', { seconds: BAN_TIMEOUT / 1000 });
}

// ── START COUNTDOWN ──
function emitToRoom(roomId, event, data) {
  // Broadcast to socket room + direct to stored player sockets (tournament safety)
  io.to('room_' + roomId).emit(event, data);
  const room = rooms[roomId];
  if (room && room.teams) {
    [...(room.teams[0]||[]), ...(room.teams[1]||[])].forEach(p => {
...
const ROOM_TIMEOUT_MS = 90 * 60 * 1000; // 90 minutes
const roomTimeouts = {};

function scheduleRoomTimeout(roomId) {
  if (roomTimeouts[roomId]) clearTimeout(roomTimeouts[roomId]);
  roomTimeouts[roomId] = setTimeout(() => {
    const room = rooms[roomId];
    if (!room || room.resultDeclared) { delete roomTimeouts[roomId]; return; }
    if (room.status !== 'playing') { delete roomTimeouts[roomId]; return; }
    room.resultDeclared = true;
    room.status = 'cancelled';
    io.to('room_' + roomId).emit('chat_msg', { author: 'Système', team: 'system', text: '⌛ Room fermée automatiquement après 90 minutes d\'inactivité.' });
    io.to('room_' + roomId).emit('room_closed_by_admin', { roomId, reason: 'timeout' });
    console.log(`[ROOM_TIMEOUT] roomId=${roomId} auto-closed after 90min`);
    setTimeout(() => { archiveRoom(roomId); delete roomTimeouts[roomId]; }, 5000);
  }, ROOM_TIMEOUT_MS);
}
...
function startRoomCountdown(roomId) {
  ...
  emitToRoom(roomId, 'countdown_start', { seconds: count });

  const interval = setInterval(() => {
    count--;
    emitToRoom(roomId, 'countdown_tick', { seconds: count });
    if (count <= 0) {
      ...
      if (room.mode === '1v1' || room.mode === '2v2' || room.mode === '3v3') {
        // 10s de discussion entre capitaines avant le vote d'arme
        room.status = 'captain_discuss';
        emitToRoom(roomId, 'captain_discuss', { seconds: CAPTAIN_DISCUSS_TIMEOUT / 1000 });
        emitToRoom(roomId, 'chat_msg', { author: 'Système', team: 'system', text: '💬 10 secondes pour vous concerter sur l\'arme !' });
        room.discussTimer = setTimeout(() => {
          const r = rooms[roomId];
          if (!r) return;
          r.status = 'weapon_vote';
          r.weaponVotes = {};
          emitToRoom(roomId, 'weapon_vote_start', { weapons: WEAPONS });
          emitToRoom(roomId, 'chat_msg', { author: 'Système', team: 'system', text: '🔫 Choisissez votre arme ! 15 secondes pour voter.' });
          r.weaponTimer = setTimeout(() => resolveWeaponVote(roomId), WEAPON_VOTE_TIMEOUT);
        }, CAPTAIN_DISCUSS_TIMEOUT);
      } else {
        room.status = 'ban_phase';
        emitToRoom(roomId, 'ban_phase', { turn: 0, team: 1, mapsLeft: MAPS.length });
        emitToRoom(roomId, 'chat_msg', { author: 'Système', team: 'system', text: '🗺️ Phase de ban ! Équipe 1 commence.' });
        startBanTimer(roomId);
      }
    }
  }, 1000);
...
app.get('/api/stats', (req, res) => {
  const data = db.loadDB();
  const onlineSockets = [...io.sockets.sockets.values()].filter(s => s.userId).length;
  const activeRooms = Object.values(rooms).filter(r => r.status === 'playing' || r.status === 'ban_phase' || r.status === 'weapon_vote').length;
  const totalMatches = data.users.reduce((sum, u) => sum + ((u.matchHistory || []).length), 0) / 2 | 0;
  res.json({
    totalPlayers: data.users.filter(u => !u.isBot).length,
    totalUsers: data.users.filter(u => !u.isBot).length,
    onlinePlayers: onlineSockets,
    onlineNow: onlineSockets,
    activeRooms,
    totalMatches
  });
});
...
// REST Auth - Login (returns JWT, no password in localStorage)
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (user.banned) return res.status(403).json({ error: '🚫 Compte banni. Raison : ' + (user.banReason || 'violation des règles') });
    const valid = await bcrypt.compare(password, user.password);
    if (!valid) return res.status(401).json({ error: 'Mot de passe incorrect' });
    const token = jwt.sign({ userId: user.id, pseudo: user.pseudo }, JWT_SECRET, { expiresIn: '30d' });
    const stats = user.stats || db.defaultStats();
    res.json({
      token,
      user: {
        id: user.id, pseudo: user.pseudo, stats,
        avatar: user.avatar || null, banner: user.banner || null,
        isPremium: !!user.isPremium, premiumUntil: user.premiumUntil || null,
        ...
        clanId: user.clanId || null, friends: user.friends || [],
        muted: !!user.muted, muteUntil: user.muteUntil || null,
        coins: user.coins || 0
      }
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
...
// REST Auth - Register (returns JWT)
app.post('/api/auth/register', async (req, res) => {
  try {
    const { pseudo, password, referralCode } = req.body;
    if (!pseudo || !password) return res.status(400).json({ error: 'Champs manquants' });
    if (pseudo.length < 3) return res.status(400).json({ error: 'Pseudo trop court (3 min)' });
    if (db.getUserByPseudo(pseudo)) return res.status(409).json({ error: 'Pseudo déjà pris' });
    ...
    if (!isLocalhost && existing.length >= 2) return res.status(429).json({ error: 'Trop de comptes créés depuis cette adresse' });
    ...
    res.status(201).json({
      token,
      user: {
        id: user.id, pseudo: user.pseudo, stats: user.stats || db.defaultStats(),
        avatar: null, banner: null, isPremium: false, premiumUntil: null,
        isAdmin: ADMIN_PSEUDOS.includes(pseudo), isContent: false,
        isFondateur: false, fondateurDate: null,
        referralCode: user.referralCode || pseudo,
        matchHistory: [], clanId: null, friends: [],
...
// REST Auth - Verify token & get fresh user data
  ...
  try {
    ...
    res.json({
      user: {
        ...
        hidePremiumBadge: !!user.hidePremiumBadge,
...
app.post('/api/profile/toggle-premium-badge', (req, res) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ error: 'Non autorisé' });
  try {
    const token = authHeader.split(' ')[1];
    ...
    if (!user) return res.status(404).json({ error: 'Introuvable' });
    user.hidePremiumBadge = !user.hidePremiumBadge;
    db.saveDB(data);
    res.json({ ok: true, hidePremiumBadge: !!user.hidePremiumBadge });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
});

// ── MISSIONS SYSTEM ──
const MISSION_TYPES = [
  { id: 'win5',       label: 'Gagner 5 parties',                    reward: 50,  target: 5 },
  { id: 'streak3',    label: 'Faire un streak de 3 victoires',       reward: 75,  target: 3 },
  { id: 'play10',     label: 'Jouer 10 parties',                     reward: 30,  target: 10 },
  { id: 'nodeath',    label: 'Gagner sans avoir perdu ce jour',       reward: 100, target: 1 },
  { id: 'play3modes', label: 'Jouer dans 3 modes différents',        reward: 60,  target: 3 },
];
...
function pickRandomMissions() {
  const shuffled = [...MISSION_TYPES].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, 3).map(m => ({ id: m.id, progress: 0, claimed: false }));
}

function getMissionProgress(user) {
  // Compute daily progress from match history
  const todayStart = new Date(); todayStart.setHours(0,0,0,0);
  const todayMatches = (user.matchHistory || []).filter(m => new Date(m.date) >= todayStart);
  ...
  const modesPlayed = new Set(todayMatches.map(m => m.mode)).size;
  const currentStreak = Math.max(...Object.values(user.stats || {}).map(s => s.currentStreak || 0), 0);

  return {
    win5:       Math.min(todayWins, 5),
    streak3:    Math.min(currentStreak, 3),
    play10:     Math.min(todayGames, 10),
    nodeath:    (todayWins >= 1 && todayLosses === 0) ? 1 : 0,
    play3modes: Math.min(modesPlayed, 3),
  };
}
...
app.get('/api/missions', (req, res) => {
  ...
  try {
    ...
    if (!isSameDay(user.lastMissionReset)) {
      user.dailyMissions = pickRandomMissions();
    ...
    const progress = getMissionProgress(user);
    const missions = user.dailyMissions.map(m => {
      const def = MISSION_TYPES.find(t => t.id === m.id);
      return {
        id: m.id,
    ...
    res.json({ missions, coins: user.coins || 0, msUntilReset });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
});
...
app.post('/api/missions/claim', (req, res) => {
  ...
  try {
    ...
    if (mission.claimed) return res.status(400).json({ error: 'Déjà réclamée' });

    const def = MISSION_TYPES.find(t => t.id === missionId);
    if (!def) return res.status(404).json({ error: 'Type de mission inconnu' });

    const progress = getMissionProgress(user);
    const currentProgress = progress[missionId] ?? 0;
    if (currentProgress < def.target) return res.status(400).json({ error: 'Mission non complétée' });

    mission.claimed = true;
    user.coins = (user.coins || 0) + def.reward;
    db.saveDB(data);
    // Give XP for completing a mission
    giveXP(user.id, 75);

    res.json({ ok: true, coins: user.coins, reward: def.reward });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
});

// ── BATTLE PASS SYSTEM ──
const BP_XP_PER_LEVEL = 150;
const BP_MAX_LEVEL = 50;

// Avatar frame definitions available as BP rewards + shop
const FRAME_DEFS = {
  glow_red:    { label: 'Bordure Feu',          emoji: '🔴' },
  glow_blue:   { label: 'Bordure Glace',         emoji: '🔵' },
  rainbow:     { label: 'Bordure Arc-en-ciel',   emoji: '🌈' },
  fire:        { label: 'Bordure Flammes',        emoji: '🔥' },
  diamond:     { label: 'Bordure Diamant',        emoji: '💎' },
  elite:       { label: 'Bordure Elite',          emoji: '⚡' },
  gold_prem:   { label: 'Bordure Dorée',          emoji: '✨' },
  neon_green:  { label: 'Bordure Néon',           emoji: '💚' },
  plasma:      { label: 'Bordure Plasma',         emoji: '🟣' },
  void_dark:   { label: 'Bordure Void',           emoji: '🌑' },
  cosmic:      { label: 'Bordure Cosmique',       emoji: '🌌' },
  electric:    { label: 'Bordure Électrique',     emoji: '⚡' },
  sunset:      { label: 'Bordure Coucher de Soleil', emoji: '🌅' },
};

// Frame rewards at specific levels (free + premium)
const FRAME_REWARDS = {
  5:  { track: 'free',    frame: 'glow_red' },
  8:  { track: 'premium', frame: 'neon_green' },
  12: { track: 'free',    frame: 'glow_blue' },
  15: { track: 'premium', frame: 'rainbow' },
  18: { track: 'free',    frame: 'fire' },
  22: { track: 'premium', frame: 'plasma' },
  25: { track: 'free',    frame: 'sunset' },
  30: { track: 'premium', frame: 'diamond' },
  35: { track: 'free',    frame: 'void_dark' },
  38: { track: 'premium', frame: 'electric' },
  42: { track: 'free',    frame: 'gold_prem' },
  45: { track: 'premium', frame: 'cosmic' },
  50: { track: 'free',    frame: 'elite' },
};

const BP_REWARDS = Array.from({ length: 50 }, (_, i) => {
  const level = i + 1;
  // Every level has coins on free track — no null slots
  const freeCoins = level % 10 === 0 ? level * 4 : (level % 5 === 0 ? level * 2 : (level % 2 === 0 ? 20 : 15));
  // Premium track scales higher
  const premCoins = level % 10 === 0 ? Math.max(150, level * 5) : (level % 5 === 0 ? 80 : (level % 2 === 0 ? 45 : 30));

  const frameReward = FRAME_REWARDS[level];
  let freeReward = { type: 'coins', amount: freeCoins };
  let premReward = { type: 'coins', amount: premCoins };

  if (frameReward) {
    const def = FRAME_DEFS[frameReward.frame];
    const frameObj = { type: 'frame', frame: frameReward.frame, label: def.label, emoji: def.emoji };
    if (frameReward.track === 'free') {
      freeReward = frameObj;
    } else {
      premReward = frameObj;
    }
  ...
  return { level, free: freeReward, premium: premReward };
});

function giveXP(userId, xpAmount) {
  const data = db.loadDB();
  const user = data.users.find(u => u.id === userId);
  if (!user) return;
  const oldXp = user.xp || 0;
  const oldLevel = Math.min(Math.floor(oldXp / BP_XP_PER_LEVEL), BP_MAX_LEVEL);
  user.xp = Math.min(oldXp + xpAmount, BP_MAX_LEVEL * BP_XP_PER_LEVEL);
  const newLevel = Math.min(Math.floor(user.xp / BP_XP_PER_LEVEL), BP_MAX_LEVEL);
  if (!user.battlePassLevel) user.battlePassLevel = 0;
  user.battlePassLevel = newLevel;
  db.saveDB(data);
  // Emit xp_gained to the player
  const ps = [...io.sockets.sockets.values()].find(s => s.userId === userId);
  if (ps) {
    ps.emit('xp_gained', {
      xpGained: xpAmount,
      ...
      nextLevelXp: Math.min((newLevel + 1) * BP_XP_PER_LEVEL, BP_MAX_LEVEL * BP_XP_PER_LEVEL),
    });
  }
}
...
app.get('/api/battle-pass', (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ error: 'Utilisateur introuvable' });
    const xp = user.xp || 0;
    const level = Math.min(Math.floor(xp / BP_XP_PER_LEVEL), BP_MAX_LEVEL);
    const levelXp = xp % BP_XP_PER_LEVEL;
    res.json({
      xp, level, levelXp,
      xpPerLevel: BP_XP_PER_LEVEL,
      maxLevel: BP_MAX_LEVEL,
      premium: !!user.battlePassPremium,
      claimedRewards: user.claimedBPRewards || [],
      coins: user.coins || 0,
      rewards: BP_REWARDS,
    });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
...
app.post('/api/battle-pass/claim', (req, res) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ error: 'Non autorisé' });
  try {
    const token = authHeader.split(' ')[1];
    ...
    const user = data.users.find(u => u.id === decoded.userId);
    if (!user) return res.status(404).json({ error: 'Utilisateur introuvable' });
    const { level, track } = req.body; // track: 'free' or 'premium'
    if (!level || !track) return res.status(400).json({ error: 'Paramètres manquants' });
    const userLevel = Math.min(Math.floor((user.xp || 0) / BP_XP_PER_LEVEL), BP_MAX_LEVEL);
    if (level > userLevel) return res.status(400).json({ error: 'Niveau pas encore atteint' });
    if (track === 'premium' && !user.battlePassPremium) return res.status(403).json({ error: 'Passe premium requis' });
    const claimKey = `${level}_${track}`;
    if (!user.claimedBPRewards) user.claimedBPRewards = [];
    if (user.claimedBPRewards.includes(claimKey)) return res.status(400).json({ error: 'Déjà réclamé' });
    const rewardDef = BP_REWARDS.find(r => r.level === level);
    if (!rewardDef) return res.status(404).json({ error: 'Niveau introuvable' });
    const reward = rewardDef[track];
    if (!reward) return res.status(404).json({ error: 'Pas de récompense pour ce niveau/track' });
    user.claimedBPRewards.push(claimKey);
    if (reward.type === 'coins') {
      ...
      // Auto-equip if user has no frame
      if (!user.avatarFrame) user.avatarFrame = reward.frame;
    }
    db.saveDB(data);
...
app.post('/api/battle-pass/buy-premium', (req, res) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ error: 'Non autorisé' });
  try {
    const token = authHeader.split(' ')[1];
    ...
    const user = data.users.find(u => u.id === decoded.userId);
    if (!user) return res.status(404).json({ error: 'Utilisateur introuvable' });
    if (user.battlePassPremium) return res.status(400).json({ error: 'Passe premium déjà activé' });
    const cost = 500;
    if ((user.coins || 0) < cost) return res.status(400).json({ error: 'Coins insuffisants' });
    user.coins -= cost;
    user.battlePassPremium = true;
    db.saveDB(data);
    res.json({ ok: true, coins: user.coins });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
});

app.post('/api/battle-pass/equip-frame', (req, res) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ error: 'Non autorisé' });
  try {
    const token = authHeader.split(' ')[1];
    ...
    if (frame && !(user.unlockedFrames || []).includes(frame) && frame !== 'none') {
      return res.status(403).json({ error: 'Bordure non débloquée' });
    }
    user.avatarFrame = frame === 'none' ? null : frame;
    db.saveDB(data);
    res.json({ ok: true, frame: user.avatarFrame });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
});

// ── ELO BOOST SHOP ──
const ELO_BOOST_DEFS = {
  boost_x15: { price: 800,  label: 'Boost ELO ×1.5',       description: '×1.5 ELO gagné sur 5 victoires', wins: 5, multiplier: 1.5 },
  boost_x2:  { price: 1800, label: 'Boost ELO ×2',          description: '×2 ELO gagné sur 3 victoires',  wins: 3, multiplier: 2.0 },
  shield:    { price: 600,  label: 'Bouclier ELO',           description: 'Protège ton ELO lors de ta prochaine défaite', wins: 0, multiplier: 1 },
  streak_protect: { price: 400, label: 'Protection de streak', description: 'Garde ton streak même si tu perds une fois', wins: 0, multiplier: 1 },
};

app.post('/api/shop/buy-boost', (req, res) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ error: 'Non autorisé' });
  try {
    const token = authHeader.split(' ')[1];
    ...
    const def = ELO_BOOST_DEFS[boostId];
    if (!def) return res.status(400).json({ error: 'Boost introuvable' });
    if ((user.coins || 0) < def.price) return res.status(400).json({ error: 'Coins insuffisants' });
    user.coins -= def.price;
    if (!user.eloBoosts) user.eloBoosts = [];
    if (boostId === 'shield') {
      user.eloShields = (user.eloShields || 0) + 1;
    } else if (boostId === 'streak_protect') {
      user.streakProtects = (user.streakProtects || 0) + 1;
    } else {
      user.eloBoosts.push({ type: boostId, remainingWins: def.wins, multiplier: def.multiplier });
    }
    db.saveDB(data);
    res.json({ ok: true, coins: user.coins });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
...
// ── SHOP ITEMS (buyable with coins) ──
const SHOP_ITEMS = {
  avatar_radiant:   { name: 'Avatar Radiant',   price: 500,  frame: 'glow_red',   limited: true },
  avatar_phantom:   { name: 'Avatar Phantom',   price: 350,  frame: 'glow_blue'  },
  avatar_agent:     { name: 'Avatar Agent',     price: 400,  frame: 'rainbow'    },
  avatar_vandal:    { name: 'Avatar Vandal',    price: 300,  frame: 'fire'       },
  effect_fire:      { name: 'Effet Feu',        price: 600,  frame: 'plasma',    limited: true },
  effect_ice:       { name: 'Effet Glace',      price: 600,  frame: 'void_dark'  },
  effect_lightning: { name: 'Effet Foudre',     price: 750,  frame: 'cosmic'     },
};
...
app.post('/api/shop/buy-item', (req, res) => {
  ...
  try {
    ...
    if (!def) return res.status(404).json({ error: 'Article introuvable' });
    const data = db.loadDB();
    const user = data.users.find(u => u.id === decoded.userId);
    if (!user) return res.status(404).json({ error: 'Utilisateur introuvable' });
    if (!user.ownedItems) user.ownedItems = [];
    if (user.ownedItems.includes(itemId)) return res.status(400).json({ error: 'Article déjà possédé' });
    if ((user.coins || 0) < def.price) return res.status(400).json({ error: `Coins insuffisants (${def.price} requis)` });
    user.coins -= def.price;
    user.ownedItems.push(itemId);
    // Unlock the frame associated with this item
    if (!user.unlockedFrames) user.unlockedFrames = [];
    if (!user.unlockedFrames.includes(def.frame)) user.unlockedFrames.push(def.frame);
    if (!user.avatarFrame) user.avatarFrame = def.frame;
    db.saveDB(data);
    res.json({ ok: true, coins: user.coins, frame: def.frame, frameDef: FRAME_DEFS[def.frame] });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
});
...
app.get('/api/shop/inventory', (req, res) => {
  ...
  try {
    ...
    const user = data.users.find(u => u.id === decoded.userId);
    if (!user) return res.status(404).json({ error: 'Utilisateur introuvable' });
    const ownedItems = (user.ownedItems || []).map(id => {
      const def = SHOP_ITEMS[id];
      return def ? { id, name: def.name, price: def.price, frame: def.frame, frameDef: FRAME_DEFS[def.frame] } : null;
    }).filter(Boolean);
    const unlockedFrames = (user.unlockedFrames || []).map(frame => ({
      frame, ...FRAME_DEFS[frame]
    }));
    res.json({ ownedItems, unlockedFrames, equippedFrame: user.avatarFrame || null, coins: user.coins || 0 });
  } catch(e) { res.status(401).json({ error: 'Token invalide' }); }
});

// ── WAGER ROOMS LOBBY ──
function getWagerRoomsData() {
  const result = {};
  // Waiting rooms (1v1, 2v2, etc.)
  for (const r of Object.values(rooms)) {
    if (r.status !== 'waiting') continue;
    const w = r.wager || 0;
    const key = `${r.mode}_${w}`;
    if (!result[key]) result[key] = { mode: r.mode, wager: w, count: 0 };
    result[key].count++;
  }
  // Solo queue
  for (const [mode, queue] of Object.entries(soloQueue)) {
    for (const entry of queue) {
      const w = entry.wager || 0;
      const key = `${mode}_${w}`;
      if (!result[key]) result[key] = { mode, wager: w, count: 0 };
      result[key].count++;
    }
  }
  return Object.values(result).filter(e => e.count > 0).sort((a, b) => a.wager - b.wager || a.mode.localeCompare(b.mode));
}

function broadcastWagerRooms() {
  io.emit('wager_rooms_update', getWagerRoomsData());
}
...
io.on('connection', (socket) => {
  ...
  socket.on('register', async ({ pseudo, password, referralCode }) => {
    try {
      ...
      const ip = normalizeIp(socket.handshake.headers['x-forwarded-for'] || socket.handshake.address);
      const isLocalhost = !ip || ip === '127.0.0.1' || ip === '::1' || ip === '::ffff:127.0.0.1';
      const existing = db.getIpAccounts(ip);
      if (!isLocalhost && existing.length >= 2) return socket.emit('auth_error', 'Trop de comptes créés depuis cette adresse');
      ...
      socket.isContent = CONTENT_PSEUDOS.includes(pseudo) || !!(user && user.isContent);
  ...
  socket.on('login', async ({ pseudo, password }) => {
    try {
      ...
      socket.avatar = user.avatar || null;
      // Update IP on every login (captures new IPs, fixes old null accounts)
      const loginIp = normalizeIp(socket.handshake.headers['x-forwarded-for'] || socket.handshake.address);
      if (loginIp && loginIp !== normalizeIp(user.ip)) {
        const loginDbData = db.loadDB();
        const loginDbUser = loginDbData.users.find(u => u.id === user.id);
        if (loginDbUser) { loginDbUser.ip = loginIp; user.ip = loginIp; db.saveDB(loginDbData); }
      }
      // Generate referralCode for old accounts that don't have one yet
      if (!user.referralCode) {
        const dbData = db.loadDB();
        const dbUser = dbData.users.find(u => u.id === user.id);
        if (dbUser) {
          // Use pseudo as referral "code" — no random code needed
          dbUser.referralCode = user.pseudo;
          user.referralCode = user.pseudo;
          db.saveDB(dbData);
          // Sync to PostgreSQL immediately so it survives redeployment
          if (process.env.DATABASE_URL) {
            const { Pool } = require('pg');
            const pgPool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
            pgPool.query('UPDATE users SET referral_code = $1 WHERE id = $2', [newCode, user.id])
              .catch(e => console.error('[PG] referral code sync:', e.message))
              .finally(() => pgPool.end());
          }
        }
      }
      socket.emit('auth_ok', {
        ...
        matchHistory: (user.matchHistory || []).slice(0, 20),
        isFondateur: !!user.isFondateur,
        fondateurDate: user.fondateurDate || null
      });
    } catch(e) { socket.emit('auth_error', 'Erreur: ' + e.message); }
  });
  ...
  socket.on('auth_with_token', ({ token }) => {
    try {
      const decoded = jwt.verify(token, JWT_SECRET);
      const user = db.getUserById(decoded.userId);
      if (!user || user.banned) return socket.emit('auth_error', user?.banned ? '🚫 Compte banni' : 'Token invalide');
      socket.userId = user.id;
      socket.pseudo = user.pseudo;
      socket.isAdmin = ADMIN_PSEUDOS.includes(user.pseudo);
      socket.isContent = CONTENT_PSEUDOS.includes(user.pseudo) || !!user.isContent;
      socket.isMuted = !!user.muted && (!user.muteUntil || user.muteUntil > Date.now());
      socket.muteUntil = user.muteUntil || null;
      socket.isPremium = !!user.isPremium;
      socket.avatar = user.avatar || null;
      const stats = user.stats || db.defaultStats();
      socket.emit('auth_ok', {
        pseudo: user.pseudo, elo: stats['2v2']?.elo || 500, stats,
        isAdmin: socket.isAdmin, isContent: socket.isContent,
        avatar: user.avatar || null, banner: user.banner || null,
    ...
    } catch(e) { socket.emit('auth_error', 'Token invalide ou expiré'); }
  });
  ...
  socket.on('rejoin_room', ({ roomId, userId }) => {
    ...
    socket.emit('room_ready', {
      roomId,
      mode: room.mode,
      team1: room.teams[0].map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, isPremium: !!p.isPremium, isContent: !!p.isContent, avatarFrame: p.avatarFrame || null })),
      team2: room.teams[1].map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, isPremium: !!p.isPremium, isContent: !!p.isContent, avatarFrame: p.avatarFrame || null })),
      captains: capPseudos,
  ...
  socket.on('get_profile', ({ pseudo }) => {
    ...
    socket.emit('profile_data', { id: user.id, pseudo: user.pseudo, stats, winrate, totalWins, totalLosses, avatar: user.avatar || null, banner: user.banner || null, isPremium: !!user.isPremium, isContent: !!(CONTENT_PSEUDOS.includes(user.pseudo) || user.isContent), isFondateur: !!user.isFondateur, fondateurDate: user.fondateurDate || null, referralCode: user.referralCode || null, matchHistory: (user.matchHistory || []) });
  });
  ...
  socket.on('spawn_bots', ({ mode }) => {
    ...
    io.to('group_' + code).emit('group_updated', { players: group.players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null })), mode: group.mode });
    socket.emit('group_created', { code, mode: group.mode, players: group.players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null })) });
    ...
    const payload = {
      ...
      team1: room.teams[0].map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null })),
      team2: room.teams[1].map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null })),
  ...
  // ── BOT MATCH (accessible à tous, pour tester l'interface) ──
  socket.on('join_bot_match', ({ mode }) => {
    if (!socket.userId) return socket.emit('notify_error', 'Non authentifié');

    const validMode = ['1v1','2v2','3v3','5v5'].includes(mode) ? mode : '2v2';
    const teamSize = getTeamSize(validMode);
    const myElo = getModeElo(socket.userId, validMode);

    // Créer un groupe solo pour l'utilisateur
    const myGroupCode = generateCode();
    const myPlayer = {
      id: socket.userId,
      pseudo: socket.pseudo,
      elo: myElo,
      avatar: socket.avatar || null,
      stats: db.getUserById(socket.userId)?.stats || null,
      socketId: socket.id,
      isBot: false,
    };
    // Remplir le reste de l'équipe avec des bots alliés si mode > 1v1
    const myTeamPlayers = [myPlayer];
    for (let i = 1; i < teamSize; i++) {
      myTeamPlayers.push({
        id: 'bot_ally_' + i + '_' + Date.now(),
        pseudo: 'AllyBot' + i,
        elo: 500,
        avatar: null,
        stats: null,
    ...
    groups[myGroupCode] = { mode: validMode, players: myTeamPlayers };

    // Créer l'équipe adverse (tous bots)
    const oppGroupCode = generateCode();
    const oppPlayers = Array.from({ length: teamSize }, (_, i) => ({
      id: 'bot_opp_' + i + '_' + Date.now(),
      pseudo: 'Bot#' + (i + 1),
      elo: 500,
      avatar: null,
      stats: { '1v1': {elo:500,wins:0,losses:0}, '2v2': {elo:500,wins:0,losses:0}, '3v3': {elo:500,wins:0,losses:0}, '5v5': {elo:500,wins:0,losses:0} },
      socketId: null,
      isBot: true,
    }));
    groups[oppGroupCode] = { mode: validMode, players: oppPlayers };

    // Créer la room
    const roomId = 'BOT_' + generateCode(4);
    const room = {
      id: roomId,
      mode: validMode,
      teams: [myTeamPlayers, oppPlayers],
      chat: [],
      mapBans: [],
      banTurn: 0,
      status: 'waiting',
    ...
    socket.join('room_' + roomId);
    socket.roomId = roomId;
    room.captains = [socket.userId, null];
    room.startedAt = Date.now();

    const payload = {
      roomId,
      mode: validMode,
      team1: myTeamPlayers.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar, stats: p.stats, isPremium: false })),
      team2: oppPlayers.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar, stats: p.stats, isPremium: false })),
      captains: [socket.pseudo, null],
      waiting: false,
    };

    socket.emit('room_ready', payload);
    socket.emit('chat_msg', { author: 'Système', team: 'system', text: '🤖 Partie test vs Bots — commence !' });
    startRoomCountdown(roomId);
  });

  // ── CREATE GROUP ──
  socket.on('invite_to_group', ({ targetPseudo }) => {
    if (!socket.userId || !targetPseudo) return;
    // Find target socket
    const targetSocket = [...io.sockets.sockets.values()].find(s => s.pseudo === targetPseudo);
    if (!targetSocket) {
      socket.emit('notify_error', targetPseudo + ' n\'est pas en ligne');
      return;
    }
    // Find inviter's group code
    let groupCode = null;
    for (const [code, group] of Object.entries(groups)) {
      if (group.players.some(p => p.id === socket.userId)) { groupCode = code; break; }
    }
    if (!groupCode) {
      socket.emit('notify_error', 'Tu n\'es pas dans un groupe');
      return;
    }
    targetSocket.emit('group_invite_received', {
      fromPseudo: socket.pseudo,
      code: groupCode
    });
  });
  ...
  socket.on('create_group', ({ mode }) => {
    ...
    groups[code] = {
      ...
      players: [{ id: socket.userId, pseudo: socket.pseudo, elo, avatar: userRecord?.avatar || null, stats: userRecord?.stats || null, isPremium: !!userRecord?.isPremium, avatarFrame: userRecord?.avatarFrame || null, socketId: socket.id }]
    };
    socket.groupCode = code;
    socket.groupMode = mode;
    socket.join('group_' + code);
    socket.emit('group_created', { code, mode, players: groups[code].players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null })) });
  });
  ...
  socket.on('reset_group_code', () => {
    ...
    socket.emit('group_created', { code: newCode, mode: group.mode, players: group.players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null })) });
    io.to('group_' + newCode).emit('group_updated', { players: group.players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null })), mode: group.mode, newCode });
  });
  ...
  socket.on('join_group', ({ code }) => {
    ...
    const userRecord = db.getUserById(socket.userId);
    group.players.push({ id: socket.userId, pseudo: socket.pseudo, elo, avatar: userRecord?.avatar || null, stats: userRecord?.stats || null, isPremium: !!userRecord?.isPremium, avatarFrame: userRecord?.avatarFrame || null, socketId: socket.id });
    socket.groupCode = code.toUpperCase();
    socket.groupMode = group.mode;
    socket.join('group_' + code.toUpperCase());

    const publicPlayers = group.players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null }));
  ...
  socket.on('create_room', () => {
    ...
    // ── Queue cooldown check ──
    if (socket.lastMatchEndedAt && !socket.isAdmin) {
      const elapsed = Date.now() - socket.lastMatchEndedAt;
      if (elapsed < QUEUE_COOLDOWN_MS) {
        const remaining = Math.ceil((QUEUE_COOLDOWN_MS - elapsed) / 1000);
        return socket.emit('queue_cooldown', { endsAt: socket.lastMatchEndedAt + QUEUE_COOLDOWN_MS, remaining });
      }
    }
    const [code, group] = entry;
    const teamSize = getTeamSize(group.mode);
    ...
    if (group.players.length === 1 && teamSize > 1 && soloQueue[group.mode] !== undefined) {
      ...
      // Validate coins for wager in solo queue
      if (socket.wagerAmount > 0) {
        const dbP = db.getUserById(player.id);
        if (!dbP || (dbP.coins || 0) < socket.wagerAmount) {
          return socket.emit('room_error', `Coins insuffisants pour la mise de ${socket.wagerAmount} 🪙 (tu as ${dbP?.coins || 0})`);
        }
      }
      ...
      soloQueue[group.mode].push({ player, groupCode: code, elo: player.elo || 500, wager: socket.wagerAmount || 0, joinedAt: Date.now() });
      broadcastWagerRooms();
      ...
      if (soloQueue[group.mode].length >= needed) {
        // Filter by matching wager, then sort by ELO
        const wagerFiltered = soloQueue[group.mode].filter(e => (e.wager || 0) === (socket.wagerAmount || 0));
        if (wagerFiltered.length < needed) {
          // Not enough same-wager players, just add to queue
          socket.emit('chat_msg', { author: 'Système', team: 'system', text: `⏳ En attente de joueurs avec la même mise (${socket.wagerAmount || 0} 🪙)…` });
          return;
        }
        const sorted = [...wagerFiltered].sort((a, b) => a.elo - b.elo);
        ...
        const payload = {
          roomId, mode: room.mode,
          team1: team1Players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, isPremium: !!p.isPremium, isContent: !!p.isContent, avatarFrame: p.avatarFrame || null })),
          team2: team2Players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, isPremium: !!p.isPremium, isContent: !!p.isContent, avatarFrame: p.avatarFrame || null })),
    ...
    // Validate each real player has enough coins for the wager
    if (socket.wagerAmount > 0) {
      for (const p of group.players.filter(pl => !pl.isBot)) {
        const dbP = db.getUserById(p.id);
        if (!dbP || (dbP.coins || 0) < socket.wagerAmount) {
          return socket.emit('room_error', `${p.pseudo} n'a pas assez de coins pour la mise de ${socket.wagerAmount} 🪙`);
        }
      }
    }
    ...
    // Expanding ELO window: starts at ±80, grows by 40 every 45s (max ±400)
    // Uses max window between the waiting room AND the searching group
    const searcherWaitSecs = 0; // new searcher starts at base window
    function getEloWindow(room) {
      const waitSecs = (Date.now() - (room.createdAt || Date.now())) / 1000;
      return Math.min(400, 80 + Math.floor(waitSecs / 45) * 40);
    }
    const searcherWindow = Math.min(400, 80 + Math.floor(searcherWaitSecs / 45) * 40);

    // Find best matching room (closest ELO within max window of both sides)
    const searcherWager = socket.wagerAmount || 0;
    let bestMatch = null, bestDiff = Infinity;
    for (const [rId, r] of waitingRooms) {
      const roomAvg = r.avgElo || 500;
      const effectiveWindow = Math.max(getEloWindow(r), searcherWindow);
      const diff = Math.abs(roomAvg - groupAvgElo);
      // Match wager amounts for all modes
      if ((r.wager || 0) !== searcherWager) continue;
  ...
  socket.on('vote_weapon', ({ weapon }) => {
    ...
    // Track who voted (for premium priority)
    if (!room.weaponVoters) room.weaponVoters = {};
    room.weaponVoters[teamKey] = socket.userId;
  ...
  // ── ADMIN BROADCAST ──
  // ── ADMIN SEND MESSAGE TO PLAYER ──
  socket.on('admin_send_message', ({ targetPseudo, message }) => {
    if (!socket.isAdmin) return;
    if (!targetPseudo || !message) return;
    // Find target socket and emit
    let sent = false;
    io.sockets.sockets.forEach(s => {
      if (s.pseudo === targetPseudo) {
        s.emit('admin_broadcast', { message, icon: '💬' });
        sent = true;
      }
    });
    console.log(`[MSG] ${socket.pseudo} → ${targetPseudo}: ${message} (sent: ${sent})`);
  });

  socket.on('admin_broadcast', ({ message, icon }) => {
    if (!socket.isAdmin) return;
    if (!message || !message.trim()) return;
    console.log(`[BROADCAST] ${socket.pseudo}: ${message}`);
    io.emit('admin_broadcast', { message: message.trim(), icon: icon || '📢' });
  });
...
function getRankNameFromElo(elo) {
  if (elo >= 1100) return 'Radiant';
  if (elo >= 900) return 'Diamond';
  if (elo >= 750) return 'Platine';
  if (elo >= 600) return 'Or';
  return 'Silver';
}
...
function applyEloResult(winTeam, loseTeam, mode) {
  ...
  winRealPlayers.forEach(p => {
    ...
    let change = db.computeEloChange(myElo, loseAvg, true);
    // ── Apply active ELO boost ──
    const dataBoost = db.loadDB();
    const userBoost = dataBoost.users.find(u => u.id === p.id);
    if (userBoost && userBoost.eloBoosts && userBoost.eloBoosts.length > 0) {
      const activeBoost = userBoost.eloBoosts[0];
      change = Math.round(change * activeBoost.multiplier);
      activeBoost.remainingWins--;
      if (activeBoost.remainingWins <= 0) userBoost.eloBoosts.shift();
      db.saveDB(dataBoost);
    }
    ...
    // ── XP ──
    giveXP(p.id, 35);
    const freshAfter = db.getUserById(p.id);
    const streak = Math.max(...Object.values(freshAfter?.stats || {}).map(s => s.currentStreak || 0), 0);
    if (streak >= 3) giveXP(p.id, 50);
  });
  loseRealPlayers.forEach(p => {
    const myElo = getElo(p);
    let change = db.computeEloChange(myElo, winAvg, false);
    // ── ELO Shield ──
    const dataShield = db.loadDB();
    const userShield = dataShield.users.find(u => u.id === p.id);
    if (userShield && userShield.eloShields > 0) {
      change = 0;
      userShield.eloShields--;
      db.saveDB(dataShield);
      const ps2 = [...io.sockets.sockets.values()].find(s => s.userId === p.id);
      if (ps2) ps2.emit('shield_used', { message: '🛡️ Ton bouclier ELO a absorbé cette défaite !' });
    }
    eloChanges[p.id] = change;
  ...
  socket.on('vote_result', ({ myTeamWon }) => {
    console.log('[VOTE] socket.userId:', socket.userId, '| socket.roomId:', socket.roomId, '| myTeamWon:', myTeamWon);
    if (!socket.roomId) { console.log('[VOTE] BLOCKED: no roomId'); return; }
    const room = rooms[socket.roomId];
    if (!room) { console.log('[VOTE] BLOCKED: no room'); return; }
    if (room.resultDeclared) { console.log('[VOTE] BLOCKED: resultDeclared'); return; }
    const validStatuses = ['playing', 'ban_phase', 'weapon_vote'];
    if (!validStatuses.includes(room.status)) { console.log('[VOTE] BLOCKED: invalid status:', room.status); return; }
    // Block vote if room started less than 30s ago
    const VOTE_DELAY_MS = 30 * 1000;
    if (room.startedAt && (Date.now() - room.startedAt) < VOTE_DELAY_MS) {
      ...
      console.log('[VOTE] BLOCKED: too early, remaining:', remaining);
      socket.emit('vote_too_early', { remaining });
      return;
    }
    // Check if this socket is a captain
    const capIndex = (room.captains || []).indexOf(socket.userId);
    console.log('[VOTE] capIndex:', capIndex, '| room.captains:', room.captains, '| room.status:', room.status);
    if (capIndex === -1) { console.log('[VOTE] BLOCKED: not a captain'); return; }
    ...
    if (cap1 && cap2 && room.votes[cap1] !== undefined && room.votes[cap2] !== undefined) {
      // cap1Won: true = cap1 pense que sa team (team1) a gagne
      // cap2Won: true = cap2 pense que sa team (team2) a gagne
      // Accord : si team1 gagne => cap1=true ET cap2=false (son equipe a perdu)
      //          si team2 gagne => cap1=false ET cap2=true
      // Donc accord = votes opposes (l'un gagne, l'autre perd forcement)
      const cap1Won = room.votes[cap1] === true;
      const cap2Won = room.votes[cap2] === true;
      const agreed = cap1Won !== cap2Won; // toujours opposes si accord
      if (agreed) {
        ...
        // Forcer tous les joueurs dans la socket room avant d'envoyer le résultat
        [...(room.teams[0]||[]), ...(room.teams[1]||[])].filter(p => p.id).forEach(p => {
          io.sockets.sockets.forEach(s => {
            if (s.userId === p.id && !s.rooms.has('room_' + socket.roomId)) {
              s.join('room_' + socket.roomId);
              s.roomId = socket.roomId;
            }
          });
        });
  ...
  socket.on('disconnect', () => {
    ...
    if (groupEntry) {
      ...
      group.players = group.players.filter(p => p.socketId !== socket.id);
      if (group.players.length === 0) delete groups[code];
      else io.to('group_' + code).emit('group_updated', { players: group.players.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, avatarFrame: p.avatarFrame || null })), mode: group.mode });
    }
  });
  ...
  socket.on('request_rematch', () => {
    ...
    if (room.rematchRequests.size >= allPlayers.length) {
      ...
      // Captains en pseudos pour que le front puisse calculer isCaptain
      const newRoom = rooms[newRoomId];
      const allT1 = newRoom.teams[0];
      const allT2 = newRoom.teams[1];
      const cap1Pseudo = allT1.find(p => !p.isBot)?.pseudo || null;
      const cap2Pseudo = allT2.find(p => !p.isBot)?.pseudo || null;
      // Stocker les IDs pour les votes
      const cap1Id = allT1.find(p => !p.isBot)?.id || null;
      const cap2Id = allT2.find(p => !p.isBot)?.id || null;
      newRoom.captains = [cap1Id, cap2Id];
      newRoom.votes = {};
      newRoom.startedAt = null; // sera set par startRoomCountdown
      const payload = {
        roomId: newRoomId,
        mode: room.mode,
        team1: allT1.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, isPremium: !!p.isPremium, isBot: !!p.isBot })),
        team2: allT2.map(p => ({ pseudo: p.pseudo, elo: p.elo, avatar: p.avatar || null, stats: p.stats || null, isPremium: !!p.isPremium, isBot: !!p.isBot })),
        captains: [cap1Pseudo, cap2Pseudo],
        isRematch: true,
        waiting: false
      };
  ...
  socket.on('get_friends', () => {
    ...
    if (!user) return socket.emit('friends_list', []);
    ...
    socket.emit('friends_list', friendInfos);
  });

  // ── SET WAGER (all modes) ──
  socket.on('set_wager', ({ amount }) => {
    const validAmounts = [0, 50, 100, 200, 500];
    const newAmount = validAmounts.includes(Number(amount)) ? Number(amount) : 0;
    if (newAmount > 0 && socket.userId) {
      const dbUser = db.getUserById(socket.userId);
      if (!dbUser || (dbUser.coins || 0) < newAmount) {
        socket.wagerAmount = 0;
        return socket.emit('wager_error', {
          error: `Coins insuffisants — tu as ${dbUser?.coins || 0} coins, mise de ${newAmount} requise.`
        });
      }
    }
    socket.wagerAmount = newAmount;
    socket.emit('wager_set', { amount: newAmount });
  });

  socket.on('get_wager_rooms', () => {
    socket.emit('wager_rooms_update', getWagerRoomsData());
  });
  ...
  // ── CHAT IMAGE ──
  socket.on('chat_img', ({ dataUrl }) => {
    if (!socket.roomId || !rooms[socket.roomId]) return;
    if (!dataUrl || typeof dataUrl !== 'string' || !dataUrl.startsWith('data:image/')) return;
    if (dataUrl.length > 3 * 1024 * 1024) return; // 3MB max in base64
    const room = rooms[socket.roomId];
    const team = room.teams[0].some(p => p.id === socket.userId) ? 'team1' : 'team2';
    io.to('room_' + socket.roomId).emit('chat_msg', {
  ...
  // ── ADMIN MODE TOGGLE ──
  // ── CHANGE PASSWORD ──
  socket.on('change_password', async ({ newPassword }) => {
    if (!socket.userId) return socket.emit('password_changed', { ok: false, error: 'Non connecté' });
    if (!newPassword || newPassword.length < 6) return socket.emit('password_changed', { ok: false, error: 'Minimum 6 caractères' });
    try {
      const hashed = await bcrypt.hash(newPassword, 10);
      const data = db.loadDB();
      const user = data.users.find(u => u.id === socket.userId);
      if (!user) return socket.emit('password_changed', { ok: false, error: 'Utilisateur introuvable' });
      user.password = hashed;
      db.saveDB(data);
      // Sync PostgreSQL — CRITICAL: without this, password resets on redeploy
      const { Pool } = require('pg');
      if (process.env.DATABASE_URL) {
        const pgPool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
        pgPool.query('UPDATE users SET password = $1 WHERE id = $2', [hashed, socket.userId])
          .catch(e => console.error('[PG] password sync error:', e.message))
          .finally(() => pgPool.end());
      }
      socket.emit('password_changed', { ok: true });
    } catch(e) {
      socket.emit('password_changed', { ok: false, error: 'Erreur serveur' });
    }
  });
  ...
  // ── ADMIN WARN PLAYER ──
  socket.on('warn_player', ({ targetPseudo, reason }) => {
    if (!socket.isAdmin) return;
    if (!targetPseudo || !reason) return;
    const targetSocket = [...io.sockets.sockets.values()].find(s => s.pseudo === targetPseudo);
    if (targetSocket) {
      targetSocket.emit('admin_warning', { reason, admin: socket.pseudo });
    }
    io.to('admins').emit('warn_sent', { targetPseudo });
    console.log(`[WARN] ${socket.pseudo} → ${targetPseudo}: ${reason} (sent: ${!!targetSocket})`);
  });
  ...
  socket.on('admin_alert', ({ roomId, type, pseudo }) => {
    ...
    // Always confirm to the requester
    if (adminCount === 0) {
      socket.emit('alert_confirm', { msg: '⚠️ Aucun admin connecté en ce moment.', ok: false });
    } else {
      const label = type === 'report' ? '🚨 Report' : '⚖️ Demande de décision';
      socket.emit('alert_confirm', { msg: `${label} envoyé à ${adminCount} admin${adminCount > 1 ? 's' : ''}.`, ok: true });
    }
  });
  ...
  // ── ADMIN CLOSE ROOM (force-cancel) ──
  socket.on('admin_close_room', ({ roomId }) => {
    if (!socket.isAdmin) return;
    const room = rooms[roomId];
    if (!room) return socket.emit('notify_error', 'Room introuvable');
    if (room.resultDeclared) return socket.emit('notify_error', 'Room déjà terminée');
    room.resultDeclared = true;
    room.status = 'cancelled';
    clearTimeout(room.banTimer);
    io.to('room_' + roomId).emit('chat_msg', { author: 'Système', team: 'system', text: '🚫 Room fermée de force par un admin.' });
    io.to('room_' + roomId).emit('room_closed_by_admin', { roomId });
    console.log(`[ADMIN_CLOSE_ROOM] roomId=${roomId} by admin=${socket.pseudo}`);
    setTimeout(() => { archiveRoom(roomId); }, 5000);
  });

  // ── REPORT PLAYER ──
  socket.on('report_player', ({ roomId, targetPseudo, reason }) => {
    if (!socket.userId || !socket.pseudo) return;
    const room = rooms[roomId] || archivedRooms[roomId];
    const validReasons = ['toxicité', 'triche', 'AFK', 'spam', 'autre'];
    if (!validReasons.includes(reason)) reason = 'autre';
    console.log(`[REPORT] ${socket.pseudo} → ${targetPseudo} | raison: ${reason} | room: ${roomId}`);
    // Notify admins
    let adminCount = 0;
    io.sockets.sockets.forEach(s => {
      if (s.isAdmin && s.adminModeActive !== false) {
        s.emit('player_reported', { roomId, reporter: socket.pseudo, target: targetPseudo, reason, time: Date.now() });
        adminCount++;
      }
    });
    socket.emit('report_sent', { ok: true, adminCount });
    // Log it
    if (room) {
      io.to('room_' + roomId).emit('chat_msg', { author: 'Système', team: 'system', text: `🚩 Un joueur a été signalé. Les admins ont été notifiés.` });
    }
  });

  // ── ADMIN DECIDE (with draw) ──
  socket.on('admin_decide', ({ roomId, winner }) => {
    console.log('[ADMIN_DECIDE] roomId:', roomId, '| winner:', winner, '| isAdmin:', socket.isAdmin);
    if (!socket.isAdmin) { console.log('[ADMIN_DECIDE] BLOCKED: not admin'); return; }
    const room = rooms[roomId];
    if (!room) { console.log('[ADMIN_DECIDE] BLOCKED: no room. Available rooms:', Object.keys(rooms)); return; }
    if (room.resultDeclared) { console.log('[ADMIN_DECIDE] BLOCKED: resultDeclared'); return; }
    const validStatuses = ['playing', 'ban_phase', 'weapon_vote', 'waiting'];
    if (!validStatuses.includes(room.status)) { console.log('[ADMIN_DECIDE] BLOCKED: invalid status:', room.status); return; }
    room.resultDeclared = true;
    room.status = 'finished';
    clearTimeout(room.banTimer);
    if (winner === 0) {
      ...
      // Forcer tous les joueurs de la room dans le socket room (reconnexion)
      [...(room.teams[0]||[]), ...(room.teams[1]||[])].filter(p => p.id).forEach(p => {
        io.sockets.sockets.forEach(s => {
          if (s.userId === p.id && !s.rooms.has('room_' + roomId)) {
            s.join('room_' + roomId);
            s.roomId = roomId;
          }
        });
...
server.listen(PORT, async () => {
  ...
  await db.ensureSchema().catch(e => console.error('[BOOT] ensureSchema error:', e.message));
  // 2. Sync data from PG to JSON
  await db.syncFromPostgres().catch(e => console.error('[BOOT] syncFromPostgres error:', e.message));
  // 3. Restore DB from Discord if /tmp/db.json is missing or empty
...
// ── ADMIN BOT MATCH SIMULATION ──
// Simule une partie contre des bots pour tester l'ELO, le match history, etc.
app.post('/api/admin/bot-match', express.json(), (req, res) => {
  if (!isAdminReq(req)) return res.status(403).json({ error: 'Interdit' });
  try {
    const { userId, mode, outcome, botElo, teamSize } = req.body;
    if (!userId) return res.status(400).json({ error: 'userId manquant' });

    const validMode = ['1v1','2v2','3v3','5v5'].includes(mode) ? mode : '2v2';
    const won = outcome === 'win';
    const bElo = Math.max(0, Math.min(2000, parseInt(botElo) || 500));
    const size = Math.max(1, Math.min(5, parseInt(teamSize) || 1));
    ...
    if (!user.stats) user.stats = {};
    if (!user.stats[validMode]) user.stats[validMode] = { elo: 500, wins: 0, losses: 0 };

    const myElo = user.stats[validMode].elo || 500;
    const eloChange = db.computeEloChange(myElo, bElo, won);

    // Pseudo-joueurs bots pour le match history
    const bots = Array.from({ length: size }, (_, i) => ({
      id: `bot_sim_${i}`, pseudo: `BOT#${i + 1}`, isBot: true,
      stats: { [validMode]: { elo: bElo } }
    }));

    db.updateUserElo(userId, eloChange, won, validMode, won ? bots : [], won ? [] : bots, false);

    const freshUser = db.getUserById(userId);
    const newElo = freshUser?.stats?.[validMode]?.elo ?? (myElo + eloChange);

    const rankFromElo = (e) => {
      if (e >= 1100) return 'Radiant';
      if (e >= 900) return 'Diamond';
      if (e >= 750) return 'Platine';
      if (e >= 600) return 'Or';
      return 'Silver';
    };
    const oldRank = rankFromElo(myElo);
    const newRank = rankFromElo(newElo);

    // Notif socket temps réel si le joueur est connecté
    const ps = [...io.sockets.sockets.values()].find(s => s.userId === userId);
    if (ps) {
      ps.emit('elo_update', { mode: validMode, newElo, change: eloChange });
      if (oldRank !== newRank) ps.emit('rank_up', { oldRank, newRank, newElo, mode: validMode });
    }

    db.addAdminLog(
      req.headers['x-admin-pseudo'] || 'Admin', 'BOT_MATCH', user.pseudo,
      `${validMode} ${won ? 'VICTOIRE' : 'DÉFAITE'} vs bots ELO${bElo} → ${eloChange > 0 ? '+' : ''}${eloChange} (${myElo} → ${newElo})`
    );

    res.json({ ok: true, eloChange, eloBefore: myElo, eloAfter: newElo, mode: validMode, outcome, rankChange: oldRank !== newRank ? { from: oldRank, to: newRank } : null });
  } catch(e) {
    console.error('[BOT_MATCH]', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.get('/api/content/dashboard/:userId', (req, res) => {
  try {
    ...
    const user = data.users.find(u => u.id === req.params.userId);
    if (!user) return res.status(404).json({ error: 'Introuvable' });
    // Allow admin to view any content dashboard, and check isContent flexibly
    const isContentUser = CONTENT_PSEUDOS.includes(user.pseudo) || !!user.isContent || user.isAdmin;
    if (!isContentUser && !db.loadDB().users.find(u=>u.id===user.id)?.isContent) return res.status(403).json({ error: 'Non autorisé' });
...
app.post('/api/friends/remove', (req, res) => {
  ...
  try {
    ...
    // Sync to PG
    if (process.env.DATABASE_URL) {
      if (user) db.sql('UPDATE users SET friends = $1 WHERE id = $2', [JSON.stringify(user.friends), userId]).catch(()=>{});
      if (target) db.sql('UPDATE users SET friends = $1 WHERE id = $2', [JSON.stringify(target.friends), targetId]).catch(()=>{});
    }
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/clans/create', express.json(), (req, res) => {
  ...
  try {
    ...
    const user = data.users.find(u => u.id === userId);
    if (!user) return res.status(404).json({ error: 'Joueur introuvable' });
    // Premium required to create a clan
    if (!user.isPremium) return res.status(403).json({ error: 'PREMIUM_REQUIRED' });
...
function getNext14Days() {
  // Reset toutes les 14 jours depuis le premier reset
  const PERIOD_MS = 14 * 24 * 60 * 60 * 1000;
  try {
    const data = readDB();
    const lastReset = data.lastClanReset || 0;
    const next = lastReset + PERIOD_MS;
    const remaining = next - Date.now();
    return remaining > 0 ? remaining : 1000; // si dépassé, reset dans 1s
  } catch(e) {
    return PERIOD_MS; // fallback 14j
  }
}
...
function doWeeklyReset() {
  try {
    ...
    top3.forEach((clan, idx) => {
      ...
      clanMessages[clan.id].push({ pseudo: 'Système', text: `🏆 Votre clan termine #${idx+1} du classement bi-mensuel ! Tous les membres reçoivent ${premiumDays} jours Premium !`, time: Date.now(), isSystem: true });
    });
    // Reset all clan weekly points
    clans.forEach(c => { c.weeklyPoints = 0; c.bo3Wins = c.bo3Wins || 0; c.bo3Losses = c.bo3Losses || 0; });
    data.lastClanReset = Date.now(); // stocker la date du reset pour le prochain cycle
    writeDB(data);
    ...
    console.log('[Bi-Weekly Reset] Done — top3:', top3.map(c => c.name));
    // Schedule next reset
    setTimeout(doWeeklyReset, getNext14Days());
  } catch(e) { console.error('[Bi-Weekly Reset] Error:', e.message); }
}
// Schedule first reset
setTimeout(doWeeklyReset, getNext14Days());
console.log(`[Bi-Weekly Reset] Next reset in ${Math.round(getNext14Days()/1000/3600)}h`);
...
app.post('/api/tournaments/create', express.json(), (req, res) => {
  try {
    ...
    const user = data.users.find(u => u.id === userId);
    if (!user) return res.status(404).json({ error: 'Utilisateur introuvable' });
    const isContentCreator = CONTENT_PSEUDOS.includes(user.pseudo) || user.isAdmin || ADMIN_PSEUDOS.includes(user.pseudo);
    if (!isContentCreator) return res.status(403).json({ error: 'CONTENT_REQUIRED' });
    // Pas de cooldown pour les Content
    if (false) {
...
// ── ADMIN: Force-cancel a tournament ──
app.post('/api/admin/cancel-tournament', express.json(), (req, res) => {
  if (!isAdminReq(req)) return res.status(403).json({ error: 'Interdit' });
  const { tournamentId } = req.body;
  const t = tournaments[tournamentId];
  if (!t) return res.status(404).json({ error: 'Tournoi introuvable' });
  if (t.status === 'finished' || t.status === 'cancelled') return res.status(400).json({ error: 'Tournoi déjà terminé/annulé' });
  t.status = 'cancelled';
  io.emit('tournament_cancelled', { id: tournamentId, name: t.name });
  console.log(`[ADMIN] Tournoi "${t.name}" annulé par ${req.headers['x-admin-pseudo'] || 'admin'}`);
  res.json({ ok: true });
});
...
function createTournamentRoom(t, match, roundIdx, matchIdx) {
  ...
  // Send tournament_room_assigned directly to each player's socket
  // Admin players still join their room (skip join only for pure spectator admins)
  const allPlayers = [...players1, ...players2];
  const playerIds = new Set(allPlayers.map(p => p.id));
  allPlayers.filter(p => p.socketId).forEach(p => {
    const s = io.sockets.sockets.get(p.socketId);
    if (!s) return;
    // Join socket room for ALL players (even if admin, they're playing in this match)
    s.join('room_' + roomId);
    s.roomId = roomId;
    s.emit('tournament_room_assigned', basePayload);
  });

  // Give clients 1.5s to process tournament_room_assigned before starting countdown
  // Also emit chat to all players directly (in case they're not in socket room yet)
  allPlayers.filter(p => p.socketId).forEach(p => {
    const s = io.sockets.sockets.get(p.socketId);
    if (s) s.emit('chat_msg', { author: 'Système', team: 'system', text: `🏆 Match de tournoi — ${t.name} · ${match.team1} vs ${match.team2}` });
  });
  setTimeout(() => {
    // Re-join any players who might have missed the join (latency safety)
    allPlayers.forEach(p => {
      if (!p.socketId) return;
      const s = io.sockets.sockets.get(p.socketId);
      if (s) { s.join('room_' + roomId); s.roomId = roomId; } // include admin players
    });
    startRoomCountdown(roomId);
  }, 1500);
